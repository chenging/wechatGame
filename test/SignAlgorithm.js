...
(function ($) {
    ...
    //取出字符串中所有空格
    function _myTrim(str) {
        return str.replace(/\s+/g, '');
    };
    //签名算法 options 需提交的参数
    function signAlgorithm  (options){
    ...
    function md5(options, key, raw) {
      let  string=signAlgorithm(options);
        if (!key) {
            if (!raw) {
