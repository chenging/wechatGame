...
! function () {
    ...
    function c(e, t) {
        ...
        //存储来源小程序信息
        if(!wx.getStorageSync('originInfo')){
            let info=JSON.parse(JSON.stringify(wx.getLaunchOptionsSync()));
            wx.setStorageSync('originInfo',JSON.stringify(info));
        }
        x.query = qdw.analysisQuery(x.query);
